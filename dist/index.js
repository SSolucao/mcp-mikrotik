...
const LOG_LEVEL = (process.env.MIKROTIK_MCP_LOG_LEVEL ?? "info").toLowerCase();
function nowMs() {
    return Date.now();
}
function shouldLog(level) {
    ...
    const configured = (LOG_LEVEL === "silent" ? "error" : LOG_LEVEL);
...
function redactConnForLogs(conn) {
...
function summarizeJsonRpcForLogs(body) {
...
async function runRouterOsCommand(input) {
...
function createMcpServer() {
    ...
    server.tool("mikrotik__run", "Executa um comando via RouterOS API. Multi-tenant: conexao (host/porta/user/senha) vai no input da tool.", RunToolShape, async (input) => {
    ...
    server.tool("mikrotik__examples", "Exemplos de uso para comandos comuns.", {}, async () => {
        const examples = {
            printResource: {
...
async function main() {
    ...
    process.stderr.write(`[boot] http host=${host} port=${port} path=${mcpPath} authToken=${authToken ? "set" : "missing"}\n`);
